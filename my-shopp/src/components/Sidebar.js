  ...
  return (
    <div className={` ${isOpen ? 'right-0' : '-right-full'} w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
      <div className="flex items-center justify-between py-6  border-b">
        <div className="uppercase text-sm font-semibold">Sopping Bag ({itemAmount})</div>
        <div onClick={handleClose} className="cursor-pointer w-8 h-8">
          <IoMdArrowForward className="text-2xl flex justify-center items-center" />
        </div>
      </div>
      ...
      <div className=" flex flex-col gap-y-3 py-4 mt-4 ">
        <div className=" flex w-full justify-between items-center ">
          ...
          <div onClick={clearCart} className="cursor-pointer py-4 bg-red-900 text-white w-12 h-12 flex justify-center items-center text-xl "><FiTrash2 /></div>
        </div>
